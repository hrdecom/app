(() => {
  ...
  const DEFAULTS = {
    collections: [{ name: "Initial", meaning: "Jewelry featuring 26 letter variants." }],
  ...
  $("addCollection").onclick = () => { const div = document.createElement('div'); div.className="config-row collections-item"; div.style="display:flex; gap:10px; margin-bottom:10px;"; div.innerHTML=`<input type="text" class="col-name" style="flex:1; border-radius:8px; border:1px solid #ddd; padding:8px;"><textarea class="col-meaning" style="flex:2; height:45px; border-radius:8px; border:1px solid #ddd; padding:8px; font-size:12px;"></textarea><button onclick="this.parentElement.remove()" style="color:red; border:none; background:none;">×</button>`; $("collectionsList").appendChild(div); };
  $("addStyleBtn").onclick = () => { const div = document.createElement('div'); div.className="config-row headline-style-item"; div.style="display:flex; gap:10px; margin-bottom:10px;"; div.innerHTML=`<input type="text" class="style-name" style="flex:1; border-radius:8px; border:1px solid #ddd; padding:8px;"><textarea class="style-prompt" style="flex:3; height:45px; border-radius:8px; border:1px solid #ddd; padding:8px; font-size:12px;"></textarea><button onclick="this.parentElement.remove()" style="color:red; border:none; background:none;">×</button>`; $("styleButtonsEditor").appendChild(div); };
  $("addAdStyleBtn").onclick = () => { const div = document.createElement('div'); div.className="config-row ad-style-item"; div.style="display:flex; gap:10px; margin-bottom:10px;"; div.innerHTML=`<input type="text" class="ad-style-name" style="flex:1; border-radius:8px; border:1px solid #ddd; padding:8px;"><textarea class="ad-style-prompt" style="flex:3; height:45px; border-radius:8px; border:1px solid #ddd; padding:8px; font-size:12px;"></textarea><button onclick="this.parentElement.remove()" style="color:red; border:none; background:none;">×</button>`; $("adStyleButtonsEditor").appendChild(div); };
  ...
  window.saveSelections = async (type) => {
    ...
    const sel = Array.from(items).map(it => it.innerText.split('\n').pop());
    if (type === 'hl') { state.selectedHeadlines = [...new Set([...state.selectedHeadlines, ...sel])]; await fetch("/api/history", { method: "PATCH", body: JSON.stringify({ id: state.currentHistoryId, headlines: JSON.stringify(state.selectedHeadlines) }) }); renderSavedHl(); } 
    else { state.selectedAds = [...new Set([...state.selectedAds, ...sel])]; await fetch("/api/history", { method: "PATCH", body: JSON.stringify({ id: state.currentHistoryId, ad_copys: JSON.stringify(state.selectedAds) }) }); renderSavedAds(); }
    alert("Enregistré");
  };
  ...
  window.deleteSaved = async (type, i) => {
    ...
    await fetch("/api/history", { method: "PATCH", body: JSON.stringify({ id: state.currentHistoryId, [type === 'hl' ? 'headlines' : 'ad_copys']: JSON.stringify(list), [type === 'hl' ? 'headlines_trans' : 'ads_trans']: JSON.stringify(trans) }) });
  ...
  function init() {
    ...
    $("translateHlMenuBtn").onclick = (e) => { e.stopPropagation(); $("hlLangList").classList.toggle('show'); };
    $("translateAdMenuBtn").onclick = (e) => { e.stopPropagation(); $("adLangList").classList.toggle('show'); };
    ...
    $("imageInput").onchange = (e) => {
      ...
      const r = new FileReader(); r.onload = (ev) => {
        ...
        state.currentHistoryId = null; state.sessionHeadlines = []; state.sessionAds = [];
      }; r.readAsDataURL(f);
    };
